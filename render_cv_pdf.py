...
AI_DISCLAIMER_TEXTS = {
    "cs": {
        "label": "Vygenerováno AI agentem, kterého vyvinul Marek Vondra  ·  ",
        "link": "jak vzniklo moje CV",
    },
    "en": {
        "label": "Generated by an AI agent built by Marek Vondra  ·  ",
        "link": "how my CV was created",
    },
}
...
def detect_language(text):
    """Rozpozná jazyk CV podle podílu znaků s českou diakritikou.
    I anglický překlad může obsahovat pár nepřeložených vlastních jmen
    (např. "Česká spořitelna"), proto se rozhoduje podle poměru, ne podle
    pouhé přítomnosti diakritiky."""
...
class NumberedCanvas(pdfcanvas.Canvas):
    def __init__(self, *args, cv_lang="cs", **kwargs):
        pdfcanvas.Canvas.__init__(self, *args, **kwargs)
        self._saved_page_states = []
        self._lang = cv_lang if cv_lang in AI_DISCLAIMER_TEXTS else "cs"
    ...
    def _draw_footer(self, total_pages):
        ...
        disclaimer_label = AI_DISCLAIMER_TEXTS[self._lang]["label"]
        disclaimer_link_text = AI_DISCLAIMER_TEXTS[self._lang]["link"]

        font_size = 7.3
        self.setFont(FONT_NAME, font_size)
        label_w = self.stringWidth(disclaimer_label, FONT_NAME, font_size)
        link_w = self.stringWidth(disclaimer_link_text, FONT_NAME, font_size)
        ...
        self.drawString(text_x, top_y, disclaimer_label)

        link_x = text_x + label_w
        self.setFillColor(SECONDARY)
        self.drawString(link_x, top_y, disclaimer_link_text)
...
def render_md_to_pdf(md_filepath, output_pdf_filepath=None):
    ...
    print(f"🌐 Rozpoznaný jazyk CV: {lang}")
